...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/blog-item'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/blog-item', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    registerChangeInBlogItems() {
        this.eventSubscriber = this.eventManager.subscribe('blogItemListModification', (response) => this.loadAll());
    }
